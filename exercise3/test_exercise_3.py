...
from test_utils import start_app, wait_for_graph_render, check_component_exists, get_graph_data, selenium_webdriver
...
def test_graph_component_present(selenium_webdriver):
    ...
    assert check_component_exists(selenium_webdriver, "attendance-graph"), "Graph container should be present"

    # Check that visual elements are rendered in the graph
    svg_element = wait_for_graph_render(selenium_webdriver, "attendance-graph")
    assert svg_element, "Graph container should contain visual elements"

    # Check that X and Y axes are present in the graph
    assert selenium_webdriver.find_elements(By.CSS_SELECTOR, ".xaxislayer-above"), "X-axis should be rendered"
    assert selenium_webdriver.find_elements(By.CSS_SELECTOR, ".yaxislayer-above"), "Y-axis should be rendered"
...
def test_graph_component_contents(selenium_webdriver):
    ...
    wait_for_graph_render(selenium_webdriver, "attendance-graph")
